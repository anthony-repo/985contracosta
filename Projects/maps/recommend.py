...
def find_centroid(cluster):
    ...
    return [mean(x) for x in zip(*[restaurant_location(y) for y in cluster])]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    s_xy = sum([p[0] * p[1] for p in zip([(x - mean(xs)) for x in xs], [(y - mean(ys)) for y in ys])])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor_rsquared_dict = {find_predictor(user, reviewed, x)[0] : find_predictor(user, reviewed, x)[1] for x in feature_fns}
    return max(predictor_rsquared_dict.keys(), key = lambda y: predictor_rsquared_dict[y])
    # END Question 8

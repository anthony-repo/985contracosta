...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = second.map(lambda x: scheme_eval(x, env))
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals.__len__() != vals.__len__():
            raise SchemeError('The number of formal parameters and arguments values are different!')
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        if not scheme_listp(args):
            raise SchemeError('arguments are not in a listII: {0}'.format(args))
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        return scheme_truep(scheme_eval(expressions.first, env)) and do_and_form(expressions.second, env)
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    is_valid_bindings = bindings
    while is_valid_bindings is not nil:
        check_form(is_valid_bindings.first, 2, 2)
        is_valid_bindings = is_valid_bindings.second

    formals = bindings.map(lambda x: x.first)
    vals = bindings.map(lambda y: scheme_eval(y.second.first, env))
    check_formals(formals)

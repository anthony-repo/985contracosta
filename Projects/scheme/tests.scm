...
'(1 ,(+ 1 2))
; expect (1 (unquote (+ 1 2)))

'(1 `,(+ 1 2))
; expect (1 (quasiquote (unquote (+ 1 2))))

`(1 ',(+ 1 2))
; expect (1 (quote 3))

(define (compsci x y)
  (cond ((= 61 (+ x y)) `ComputerScience61A)
        (else `(Computer Science ,(+ x y) is awesome))))

(compsci 1 2)
; expect (computer science 3 is awesome)

(define (begin-and-after a b)
  (begin (define sum (+ a b))
         (define diff (- a b))
         (define prod (* a b))
         (define quot (/ a b))
         (if (< diff quot)
...
(define (logic-func a b) (or (and a b) (or a b)))
...
(cond ((cond ((= 1 2) 'Computer)
             ((= 3 3) (cond ((= 5 6) 'Science)
                            ((= 6 7) '61A))))))
; expect

(define m 2)
(define n 3)
(let ((n (+ m 1)) (m (* n 3))) (+ m n) (* m n))
; expect 27

(define computer (mu (c d) (cond ((= a c) 'computer)
                                 ((< (+ a b) d) 'science))))

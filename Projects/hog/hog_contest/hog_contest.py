...
PLAYER_NAME = 'Oink!Oink!Oink!' # Change this line!
...
def is_swap(player_score, opponent_score):
    ...
    if player_score % 10 == opponent_score % 10:
        ...
        return is_swap(player_score // 10, opponent_score //10 ) 
...
def always_roll(n):
    ...
    5
    """
    def strategy(score, opponenßt_score):
        return n
    return strategy
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, max_rolls, highest_average = 1, 1, 0

    for i in range (num_rolls, 10 + 1):
        roll_average = make_averaged(roll_dice, num_samples)(i, dice)

        if roll_average > highest_average:
            highest_average = roll_average
            max_rolls = i
        elif roll_average == highest_average:
            max_rolls = min(max_rolls, i)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(score + free_bacon(opponent_score), opponent_score):
        if score + free_bacon(opponent_score) < opponent_score:
            return 0
        if score + free_bacon(opponent_score) == opponent_score and bacon_strategy(score, opponent_score, margin, num_rolls) == 0:
            return 0

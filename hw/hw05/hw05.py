...
def make_fib():
    ...
    next_num = 1
    def fib_number():
        nonlocal curr_num, next_num

        curr_num, next_num = next_num, curr_num + next_num

        return next_num - curr_num
...
def make_withdraw(balance, password):
    ...
    True
    """
    d = {'balance': balance, 'password': password}
    incorrect = []
    ...
    def withdraw(amount, passphrase):
        ...
        if len(incorrect) < 3:
            if passphrase == password:
                check_balance = d['balance'] - amount
                if check_balance < 0:
                    return 'Insufficient funds'
                else:
                    d['balance'] = check_balance
                    return d['balance']
            else:
                incorrect.append(passphrase)
...
def make_joint(withdraw, old_password, new_password):
    ...
    def new_withdraw(amount, passphrase):
        if passphrase == old_password or passphrase == new_password:
...
class Coin:
    ...
    def worth(self):
        extra_value = Mint.current_year - self.year - 50
        return (lambda : self.cents + extra_value if extra_value > 0 else self.cents)()

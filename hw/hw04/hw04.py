...
def prune_leaves(t, vals):
    ...
    next_tree = [prune_leaves(x,vals) for x in branches(t) if prune_leaves(x, vals)]
    return tree(label(t), next_tree)
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        is_balanced = length(left(m)) * total_weight(end(left(m))) == length(right(m)) * total_weight(end(right(m)))
...
def totals_tree(m):
    ...
    if is_side(m):
        ...
        return tree(total_weight(m), [totals_tree(x) for x in branches(m)])

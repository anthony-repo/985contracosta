...
class Player(object):
    def __init__(self, deck, name):
        ...
        self.deck = deck
        self.name = name
        self.hand = [deck.draw() for x in range(0,5)]
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for i in range(0,len(player.deck.cards)):
            player.deck.cards[i].attack += other_card.attack
            player.deck.cards[i].defense += other_card.defense
            
        for i in range(0, len(opponent.deck.cards)):
            if opponent.deck.cards[0].attack == other_card.attack or opponent.deck.cards[0].defense == other_card.defense:
                opponent.deck.cards.pop(0)
       
        discarded = orig_opponent_deck_length - len(opponent.deck.cards)
        if discarded:
            #Uncomment the line below when you've finished implementing this meßthod!
